...
class FlutterFirebaseAppDistributionCommand extends UpcodeCommand with EnvironmentMixin, ApplicationMixin {
  FlutterFirebaseAppDistributionCommand(this.args, Map<String, dynamic> config) : super(config) {
    argParser
      ..addOption(
        'env',
        abbr: 'e',
  ...
  FutureOr<dynamic> run() async {
    final String newCommand = 'upcode fad upload ${args.splitAfter((String item) => item == 'flutter:fad').last.join(' ')}';
    stderr.writeln(
        'You should not use this command, it will be removed in a future version. You can replace it with: \n\n$newCommand\n\n');
    await initFirebase();
